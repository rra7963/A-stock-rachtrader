...
class ResolveInstrumentIdentityTests(unittest.TestCase):
    ...
    def test_a_share_uses_tushare_identity_without_yahoo(self):
        set_config({"data_vendors": {"core_stock_apis": "tushare"}})
        with patch(
            "tradingagents.dataflows.tushare_stock.get_instrument_identity",
            return_value={
                "company_name": "贵州茅台",
                "industry": "白酒",
                "exchange": "SSE",
            },
        ) as tushare, patch("tradingagents.agents.utils.agent_utils.yf.Ticker") as yahoo:
            identity = resolve_instrument_identity("600519.SS")

        tushare.assert_called_once_with("600519.SS")
        yahoo.assert_not_called()
        self.assertEqual(identity["company_name"], "贵州茅台")
        self.assertEqual(identity["industry"], "白酒")

    def test_a_share_fails_open_without_yahoo_fallback(self):
        set_config({"data_vendors": {"core_stock_apis": "tushare"}})
        with patch(
            "tradingagents.dataflows.tushare_stock.get_instrument_identity",
            side_effect=RuntimeError("database unavailable"),
        ), patch("tradingagents.agents.utils.agent_utils.yf.Ticker") as yahoo:
            identity = resolve_instrument_identity("600519.SH")

        self.assertEqual(identity, {})
        yahoo.assert_not_called()

    def test_a_share_non_tushare_config_does_not_require_tushare_token(self):
        set_config({"data_vendors": {"core_stock_apis": "aliyun_stock_postgres"}})
        with patch(
            "tradingagents.dataflows.aliyun_stock_postgres.get_instrument_identity",
            return_value={"company_name": "贵州茅台", "exchange": "SSE"},
        ) as aliyun, patch(
            "tradingagents.dataflows.tushare_stock.get_instrument_identity"
        ) as tushare:
            identity = resolve_instrument_identity("600519.SH")

        aliyun.assert_called_once_with("600519.SH")
