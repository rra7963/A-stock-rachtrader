...
class TestTradingMemoryLogCore:
    ...
    def test_independent_graph_memory_logs_serialize_process_local_file_io(
        self,
        tmp_path,
        monkeypatch,
    ):
        target = tmp_path / "trading_memory.md"
        first = make_log(tmp_path)
        first.store_decision("SEED", "2026-01-01", DECISION_BUY)
        logs = [make_log(tmp_path) for _ in range(8)]
        ...
        assert max_active == 1
        assert len(first.load_entries()) == 9
...
class TestDeferredReflection:
    ...
    def test_fetch_returns_routes_cross_market_benchmark_independently(self):
        """An A-share with an explicit SPY benchmark uses both providers."""
        mock_graph = MagicMock(spec=TradingAgentsGraph)
        mock_graph.config = {"data_vendors": {"core_stock_apis": "tushare"}}
        a_share_prices = pd.Series(
            [100.0, 105.0], index=pd.to_datetime(["2026-01-05", "2026-01-06"])
        )
        spy_history = _price_df([400.0, 404.0])

        with patch(
            "tradingagents.dataflows.tushare_stock.get_close_prices",
            return_value=a_share_prices,
        ) as tushare_prices, patch("yfinance.Ticker") as ticker_cls:
            ticker_cls.return_value.history.return_value = spy_history
        ...
        tushare_prices.assert_called_once_with("600519.SH", "2026-01-05", "2026-01-17")
        ticker_cls.assert_called_once_with("SPY")
        assert raw == pytest.approx(0.05)
        assert alpha == pytest.approx(0.04)
        assert days == 1

    def test_fetch_returns_non_tushare_a_share_uses_configured_vendor(self):
        """A configured PostgreSQL A-share path must not require Tushare."""
        mock_graph = MagicMock(spec=TradingAgentsGraph)
        mock_graph.config = {
            "data_vendors": {"core_stock_apis": "aliyun_stock_postgres,yfinance"}
        }
        csv_data = "\n".join(
            [
                "Date,Open,High,Low,Close,Volume,Amount",
                "2026-01-05,99,101,98,100,1,1",
                "2026-01-06,104,106,103,105,1,1",
            ]
        )
        ...
        with patch.dict(
            ...
            {"aliyun_stock_postgres": MagicMock(return_value=csv_data)},
        ), patch(
            "tradingagents.dataflows.tushare_stock.get_close_prices"
        ) as tushare_prices, patch("yfinance.Ticker") as ticker_cls:
        ...
        tushare_prices.assert_not_called()
        assert raw == pytest.approx(0.05)
        assert alpha == pytest.approx(0.04)
        assert days == 1

    def test_fetch_returns_converts_shanghai_symbols_for_yfinance(self):
        """Yahoo receives .SS while Tushare/database providers retain .SH."""
        mock_graph = MagicMock(spec=TradingAgentsGraph)
        mock_graph.config = {"data_vendors": {"core_stock_apis": "yfinance"}}
        requested_symbols = []

        def yahoo_prices(symbol, start, end):
            requested_symbols.append(symbol)
            return "\n".join(
                [
                    "Date,Open,High,Low,Close,Volume",
                    "2026-01-05,99,101,98,100,1",
                    "2026-01-06,104,106,103,105,1",
                ]
            )
        ...
        assert requested_symbols == ["600519.SS", "000001.SS"]
        assert raw == pytest.approx(0.05)
        assert alpha == pytest.approx(0.0)
        assert days == 1

    def test_fetch_returns_honors_tool_level_stock_vendor_override(self):
        mock_graph = MagicMock(spec=TradingAgentsGraph)
        mock_graph.config = {
    ...
    def test_fetch_returns_aligns_stock_and_benchmark_by_common_date(self):
        mock_graph = MagicMock(spec=TradingAgentsGraph)
        mock_graph.config = {"data_vendors": {"core_stock_apis": "tushare"}}
        ...
        benchmark_prices = pd.DataFrame(
            {"Close": [400.0, 420.0, 430.0]},
            index=pd.to_datetime(["2026-01-05", "2026-01-07", "2026-01-08"]),
        )

        with patch(
            "tradingagents.dataflows.tushare_stock.get_close_prices",
            return_value=stock_prices,
        ), patch("yfinance.Ticker") as ticker_cls:
            ticker_cls.return_value.history.return_value = benchmark_prices
            raw, alpha, days = TradingAgentsGraph._fetch_returns(
                mock_graph,
                "600519.SH",
                "2026-01-05",
        ...
        # The common end date is Jan 7: stock +30%, benchmark +5%.
        assert raw == pytest.approx(0.30)
        assert alpha == pytest.approx(0.25)
        assert days == 1

    def test_fetch_returns_aligns_cross_timezone_series_by_local_session_date(self):
        mock_graph = MagicMock(spec=TradingAgentsGraph)
        tokyo_prices = pd.DataFrame(
            {"Close": [100.0, 110.0, 120.0]},
            index=pd.date_range("2026-01-05", periods=3, tz="Asia/Tokyo"),
        )
        new_york_prices = pd.DataFrame(
            {"Close": [100.0, 105.0, 110.0]},
            index=pd.date_range("2026-01-05", periods=3, tz="America/New_York"),
        )

        with patch("yfinance.Ticker") as ticker_cls:
            ticker_cls.side_effect = lambda symbol: MagicMock(
                history=MagicMock(
                    return_value=(
                        tokyo_prices if symbol == "7203.T" else new_york_prices
                    )
                )
            )
            raw, alpha, days = TradingAgentsGraph._fetch_returns(
    ...
    def test_fetch_returns_uses_explicit_tushare_index_entry_for_benchmark(self):
        mock_graph = MagicMock(spec=TradingAgentsGraph)
        mock_graph.config = {"data_vendors": {"core_stock_apis": "tushare"}}
        dates = pd.to_datetime(["2026-01-05", "2026-01-06"])
        stock_prices = pd.Series([100.0, 105.0], index=dates)
        index_prices = pd.Series([4500.0, 4545.0], index=dates)

        with patch(
            "tradingagents.dataflows.tushare_stock.get_close_prices",
            return_value=stock_prices,
        ) as stock_loader, patch(
            "tradingagents.dataflows.tushare_stock.get_index_close_prices",
            return_value=index_prices,
        ) as index_loader:
            raw, alpha, days = TradingAgentsGraph._fetch_returns(
                mock_graph,
                "600519.SH",
        ...
        index_loader.assert_called_once_with("000300.SH", "2026-01-05", "2026-01-17")
        assert raw == pytest.approx(0.05)
        assert alpha == pytest.approx(0.04)
        assert days == 1

    def test_fetch_returns_uses_tushare_fund_entry_for_etf_benchmark(self):
        mock_graph = MagicMock(spec=TradingAgentsGraph)
        mock_graph.config = {"data_vendors": {"core_stock_apis": "tushare"}}
        dates = pd.to_datetime(["2026-01-05", "2026-01-06"])
        stock_prices = pd.Series([100.0, 105.0], index=dates)
        fund_prices = pd.Series([4.0, 4.04], index=dates)

        with patch(
            "tradingagents.dataflows.tushare_stock.get_close_prices",
            return_value=stock_prices,
        ) as stock_loader, patch(
            "tradingagents.dataflows.tushare_stock.get_fund_close_prices",
            return_value=fund_prices,
        ) as fund_loader, patch(
            "tradingagents.dataflows.tushare_stock.get_index_close_prices"
        ) as index_loader:
            raw, alpha, days = TradingAgentsGraph._fetch_returns(
                mock_graph,
        ...
        stock_loader.assert_called_once_with("600519.SH", "2026-01-05", "2026-01-17")
        fund_loader.assert_called_once_with("510300.SH", "2026-01-05", "2026-01-17")
