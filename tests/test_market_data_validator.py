...
class TestVerifiedSnapshot:
    def test_loads_ohlcv_through_configured_vendor_route(self, monkeypatch):
        calls = []
        ...
        def fake_route(method, symbol, start_date, end_date):
            ...
            return "\n".join(
                [
                    ...
                    "2026-05-12,10,11,9,10.5,1000,10500",
                    "2026-05-13,10.5,12,10,11.5,1100,12650",
                ]
            )

        monkeypatch.setattr(validator, "route_to_vendor", fake_route)

        snap = validator.build_verified_market_snapshot("688037.SS", "2026-05-13")

        assert calls
        assert calls[0][0] == "get_stock_data"
        assert calls[0][1] == "688037.SS"
        assert calls[0][3] == "2026-05-13"
