"""Persistent request-id idempotency store for the event trade-plan API."""
...
from .schemas import EventTradePlanResponse
...
class ClaimResult:
    status: ClaimStatus
    response: EventTradePlanResponse | None = None
    error_code: str | None = None


class EventPlanStore:
    """SQLite-backed state machine that never stores the raw request or credentials."""
    ...
    def claim(self, request_id: str, request_sha256: str) -> ClaimResult:
        now = _utc_now_text()
        with self._connect() as connection:
            ...
            if row is None:
                connection.execute(
                    """
                    INSERT INTO event_plan_requests (
            ...
            if row["status"] == "completed":
                response = EventTradePlanResponse.model_validate_json(row["response_json"])
                return ClaimResult("completed", response=response)
            if row["status"] == "running":
                return ClaimResult("running")
            return ClaimResult("failed", error_code=row["error_code"])

    def complete(self, response: EventTradePlanResponse) -> None:
        now = _utc_now_text()
        with self._connect() as connection:
            ...
            if cursor.rowcount != 1:
                raise RuntimeError("request is not in the expected running state")
    ...
    def fail(self, request_id: str, request_sha256: str, error_code: str) -> None:
        ...
        with self._connect() as connection:
            cursor = connection.execute(
                ...
                WHERE request_id = ? AND request_sha256 = ? AND status = 'running'
                """,
                (error_code, now, request_id, request_sha256),
            )
            if cursor.rowcount != 1:
                raise RuntimeError("request is not in the expected running state")
