...
def normalize_a_share_symbol(raw: str) -> str | None:
    """Return a canonical Tushare A-share code, or ``None`` if not recognized.

    The public input contract accepts exchange-qualified symbols and bare
    six-digit stock codes. ``.SS`` is normalized to Tushare's ``.SH`` form.
    This helper is purely syntactic and shared by the CLI, identity routing,
    and return-calculation paths.
    """
    ...
    if candidate.isdigit() and len(candidate) == 6:
        ...
        if candidate.startswith(("4", "8", "920")):
            return f"{candidate}.BJ"
    return None


def is_a_share_symbol(raw: str) -> bool:
    """Return whether ``raw`` matches the supported A-share symbol contract."""
    return normalize_a_share_symbol(raw) is not None
