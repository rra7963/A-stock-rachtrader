...
from tradingagents.graph.trading_graph import (
    TradingAgentsGraph,
    validate_portfolio_data_vendor_coverage,
)
...
def _execute_analysis_command(
    ...
    """Shared implementation for the explicit and backwards-compatible default command."""
...
def default_command(
    ...
    checkpoint: bool | None = typer.Option(
        ...
        help="Backwards-compatible root option for the default analyze command.",
    ),
    ...
    """Keep the legacy root command equivalent to explicit ``analyze``."""
...
def portfolio(
    tickers: str = typer.Argument(
        ...,
        help="2 to 20 comma-separated stock symbols.",
    ),
    ...
    max_position: float = typer.Option(
        ...
        min=0.01,
        max=100.0,
        help="Maximum percentage allocated to any one stock.",
    ),
    save_path: str | None = typer.Option(
        None,
        "--save-path",
        help="Directory for portfolio_allocation.md and portfolio_allocation.json.",
    ),
):
    """Analyze 2 to 20 stocks together and output target portfolio weights."""
    raw_tickers = [part.strip() for part in tickers.split(",") if part.strip()]
    normalized_tickers = [normalize_ticker_symbol(ticker) for ticker in raw_tickers]
    from tradingagents.dataflows.symbol_utils import normalize_a_share_symbol

    a_share_symbols = [
        normalize_a_share_symbol(ticker) for ticker in normalized_tickers
    ]
    all_a_shares = bool(a_share_symbols) and all(a_share_symbols)
    if all_a_shares:
        # Portfolio mode explicitly selects Tushare for an all-A-share set, so
        # normalize here rather than changing the legacy Yahoo-facing analyze
        # path's global ticker normalization.
        normalized_tickers = [symbol for symbol in a_share_symbols if symbol]
    else:
        # Mixed-market portfolios keep the configured Yahoo-oriented data path,
        # but each recognized A-share still needs a provider-valid symbol.  In
        # particular, bare Shanghai codes must become ``.SS`` instead of being
        # handed to Yahoo unchanged (or leaking Tushare's ``.SH`` convention).
        normalized_tickers = [
            (
                symbol[:-3] + ".SS"
                if symbol and symbol.endswith(".SH")
                else symbol or ticker
            )
            for ticker, symbol in zip(normalized_tickers, a_share_symbols, strict=True)
        ]
    if not 2 <= len(normalized_tickers) <= 20:
        raise typer.BadParameter(
            f"portfolio mode requires 2 to 20 comma-separated tickers; "
            f"received {len(normalized_tickers)}",
            param_hint="tickers",
        )
    if len({ticker.casefold() for ticker in normalized_tickers}) != len(normalized_tickers):
        raise typer.BadParameter("all portfolio tickers must be unique", param_hint="tickers")
    ...
    try:
        ...
        raise typer.BadParameter("date must use YYYY-MM-DD format", param_hint="--date") from exc
    if parsed_date > datetime.datetime.now().date():
        raise typer.BadParameter("analysis date cannot be in the future", param_hint="--date")

    config = copy.deepcopy(DEFAULT_CONFIG)
    selected_analysts = ("market", "fundamentals")
    if all_a_shares:
        # Keep A-share portfolio market data entirely on Tushare, even when an
        # older .env still contains yfinance defaults.
        config["data_vendors"]["core_stock_apis"] = "tushare"
        config["data_vendors"]["technical_indicators"] = "tushare"
        config["data_vendors"]["fundamental_data"] = "tushare"
    try:
        validate_portfolio_data_vendor_coverage(
            normalized_tickers,
            config,
    ...
    console.print(
        f"[bold cyan]Portfolio mode:[/bold cyan] analyzing "
        f"{', '.join(normalized_tickers)} for {analysis_date}"
    )

    def show_progress(ticker: str, index: int, total: int) -> None:
        console.print(f"[cyan][{index}/{total}][/cyan] Analyzing [bold]{ticker}[/bold]...")

    _, allocation = graph.propagate_portfolio(
        normalized_tickers,
        analysis_date,
    ...
    if save_path is None:
        ...
        save_path = Path.cwd() / "reports" / f"portfolio_{analysis_date}_{stamp}"
    report_path = graph.save_portfolio_report(allocation, save_path)
    console.print(f"\n[green]✓ Portfolio report saved to:[/green] {report_path.resolve()}")
